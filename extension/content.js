...
const stateManager = {
    ...
    createState() {
        ...
        // Create new state with current counter value
        const stateId = `state_${this.stateCounter++}`;
        debugLog(`Creating new state with ID ${stateId} for URL: ${window.location.href}, counter after increment: ${this.stateCounter}`);
        
        // Save the counter to persist between page navigations
    ...
    if (storedState) {
        try {
            ...
            if (state && state.isRecording && state.sessionId) {
                ...
                // Restore session ID immediately so we can try to restore the state counter
                sessionId = state.sessionId;
                userId = state.userId;
                
                // Try to restore the state counter from previous page in same session
                stateManager.restoreStateCounter();
            }
        } catch (e) {
...
function handleBeforeUnload() {
    if (isRecording) {
        ...
        // Save state counter before page unloads to persist it to the next page
        stateManager.saveStateCounter();
    }
...
function startRecording(sessionID, userID) {
    if (isRecording) {
        ...
        if (sessionID !== sessionId) {
            ...
            // Reset counter when switching to a completely new session
            stateManager.stateCounter = 0;
            stateManager.saveStateCounter();
            debugLog('Reset state counter to 0 for new recording session');
        }
        return;
    }
    ...
    // Only reset counter when starting a completely new recording session
    // If there's a saved counter for this session, restore it instead
    if (!stateManager.restoreStateCounter()) {
        stateManager.stateCounter = 0;
        stateManager.saveStateCounter();
        debugLog('Reset state counter to 0 for new recording session');
    }
